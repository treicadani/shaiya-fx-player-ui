USE [PS_UserData]
GO
/****** Object:  StoredProcedure [dbo].[usp_Try_GameLogin_Taiwan]    Script Date: 18/12/2020 13:16:39 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	select @Pw = dbo.Users_Master.Pw from PS_UserData.dbo.Users_Master where UserID = @userid

	select @success = dbo.CheckPassword(@InPassword, @Pw)

	IF EXISTS(Select * From users_master with (nolock) Where userid=@userid  And @success = 1)
		BEGIN
			--Login Success
			SET @Check = 1
			UPDATE PS_UserData.dbo.Users_Master SET Leave = 1 WHERE userid=@userid
		END
	ELSE
		BEGIN
			--Login Fail
